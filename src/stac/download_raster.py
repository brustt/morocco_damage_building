...
def download_maxar_on_roi(
                ...
                remove_full_rs:bool=True) -> List[str]: 
    ...
    for name in roi_gdf.town_name.unique():
        ...
        if isinstance(roi_buffer.geometry.iloc[0], Point):   
            roi_buffer.geometry = [box(*_.bounds) for _ in roi_buffer.buffer(buffer_size)]
        ...
        if save_items_shp:
            items_town.to_file(os.path.join(path_town_dir, f"maxar_items_{name}_{type_item}.shp"))
        ...
        for i, row in items_town.iterrows():
            ...
            logger.info(f"Save {path_rs_clip} for {name} : {delta_event} event {row['datetime']}")
